...
class CARightCenter(commands2.SequentialCommandGroup):
    def __init__(self, fuelSubsystem: CANFuelSubsystem, drivetrain: DriveSubsystem) -> None:
        super().__init__()
        self.addCommands(
            PrintCommand("CA_Right Center started"),
            ResetXY(x=3.6, y=3.0, headingDegrees=0,drivetrain=drivetrain).withTimeout(.5),
            PrintCommand("Reset just ran"),

            SwerveToPoint(x=3.38 , y=3.0, headingDegrees=45, drivetrain=drivetrain, speed=1.0).withTimeout(3.0),
            ...
            SwerveToPoint(x=3.38, y=0.7, headingDegrees=90, drivetrain=drivetrain, speed=1.0).withTimeout(6.0),
            GoToPoint(3.38, 3.0, drivetrain=drivetrain, speed=1.0),
            AimToDirection(degrees=45, drivetrain=drivetrain, speed=1.0).withTimeout(1),
            ...
            AimToDirection(degrees=270, drivetrain=drivetrain, speed=1.0).withTimeout(1),
            GoToPoint(3.38, 1.6, drivetrain=drivetrain, speed=1.0),
            AimToDirection(degrees=270, drivetrain=drivetrain, speed=1.0).withTimeout(1),

            PrintCommand("CA_Right Center finished")
        )